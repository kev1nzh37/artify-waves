import { RivuletClient } from "@repo/core"
...
export const Rivulet: React.ForwardRefRenderFunction<
  HTMLDivElement,
  Props
> = (props) => {
  ...
  const [client, setClient] = useState<RivuletClient | null>(null)

  const init = () => {
    if (!dom.current || client !== null) return
    const c = new RivuletClient({
      dom: dom.current,
    })
    setClient(c)
  }
