import { ErosionClient } from "@repo/core"
...
export const Erosion: React.ForwardRefRenderFunction<
  HTMLDivElement,
  Props
> = (props) => {
  ...
  const [client, setClient] = useState<ErosionClient | null>(null)

  const init = () => {
    if (!dom.current || client !== null) return
    const c = new ErosionClient({
      dom: dom.current,
    })
    setClient(c)
  }
